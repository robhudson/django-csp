...
==============================
When ``INCLUDE_NONCE_IN`` is configured, the nonce value is returned in the CSP headers **if it is
...
Assuming the ``INCLUDE_NONCE_IN`` list contains the ``script-src`` directive, this will result in
