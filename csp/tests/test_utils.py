...
def test_config():
    policy = build_policy(config={"DIRECTIVES": {"default-src": ["'none'"], "img-src": ["'self'"]}})
...
@override_settings(CONTENT_SECURITY_POLICY=[{"INCLUDE_NONCE_IN": ["script-src", "style-src"]}])
...
@override_settings(CONTENT_SECURITY_POLICY=[{"DIRECTIVES": {"default-src": ["'self'"]}}])
def test_nonce_include_in_absent():
    config = settings.CONTENT_SECURITY_POLICY[0]
    config.pop("INCLUDE_NONCE_IN", None)
    policy = build_policy(config=config, nonce="abc123")
    policy_eq("default-src 'self' 'nonce-abc123'", policy)
